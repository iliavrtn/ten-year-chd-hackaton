...
def write_tables(save_path, X_train, y_train, clf, clf_type, task_type, unc_type):
    """
    ...
    # f'{clf_type}/model_{clf_type}_{unc_type}.csv
    constraint_add.to_csv(save_path + f'/{clf_type}/model_{clf_type}_{unc_type}.csv', index=False)
    print(f'{clf_type} tables saved.')


def base_model(u, F_countefactual_features, F_binary, F_integers, F_coherence, F_immutability, F_lager_than,
               F_positivity):
    """
    ...
    model.x = Var(F_countefactual_features, domain=Reals,
                  name=['ce_%s' % str(ce) for ce in F_countefactual_features])  # counterfactual features
    ...
    for cat in F_coherence.keys():
        model.add_component('coherence_' + cat, Constraint(expr=sum(model.x[i] for i in F_coherence[cat]) == 1))

    def immutability(model, i):
        # print()
        return model.x[i] == u[i].item()

    model.add_component('immutability', Constraint(F_immutability, rule=immutability))

    def larger(model, i):
        return model.x[i] >= u[i].item()

    model.add_component('larger_than', Constraint(F_lager_than, rule=larger))

    def positive(model, i):
        return model.x[i] >= 0

    model.add_component('positive', Constraint(F_positivity, rule=positive))

    return model


def find_maxrad(x_, clf_type, save_path, F_countefactual_features, F_binary, F_integer, F_coherence, F_immutability,
                F_lager_than,
                F_positivity, class_, obj_type):
    """
    ...
    """
    clf_path = save_path + '/' + clf_type + f'/model_{clf_type}_{obj_type}.csv'
    mfile = pd.read_csv(clf_path)
    model = base_model(x_, F_countefactual_features, F_binary, F_integer, F_coherence, F_immutability, F_lager_than,
                       F_positivity)
    ...
    if clf_type in ['linear', 'svm']:
        model = constraints_linear(model, 'prediction', class_, mfile, F_countefactual_features, 0, None, lb, ub)
    elif clf_type == 'cart':
        model = constraints_tree(model, 'prediction', mfile, F_countefactual_features, 0, None, lb, ub,
                                 adversarial_algorithm_bool=False, S=[], adv_prob=False)  # check it
    elif clf_type == 'rf':
        model = constraints_rf(model, 'prediction', mfile, F_countefactual_features, 0, None, lb, ub,
                               adversarial_algorithm_bool=False, S=[], adv_prob=False)
    elif clf_type == 'gbm':
        model = constraints_gbm(model, 'prediction', mfile, F_countefactual_features, 0, None, lb, ub,
                                adversarial_algorithm_bool=False, S=[], adv_prob=False)
    elif clf_type == 'mlp':
        model = constraints_mlp(model, 'prediction', mfile, F_countefactual_features, lb, ub, adv_prob=True,
                                S=[])  # check it
...
def constraints_linear(model, outcome, class_, linear_table, F_countefactual_features, rho, unc_type, lb, ub):
    """
    ...
    model.add_component('linear_model', Constraint(
        expr=model.y[outcome] == sum(
            model.x[i] * coeff.loc[i] for i in F_countefactual_features) + intercept + robust_term))
...
def constraints_tree(model, outcome, tree_table, F_countefactual_features, rho, unc_type, lb, ub, adversarial_algorithm_bool, S, adv_prob):
    """
    ...
    if not adversarial_algorithm_bool:  # the robust counterfactual explanations will be in one of the leaves
        def constraintsTree_1(model, j):
            if unc_type == 'l2':
                ...
                robust_term = 0
            return sum(model.x[i] * coeff.loc[j, i] for i in F_countefactual_features) + robust_term <= intercept.iloc[j] + M * (
        ...
        if adv_prob:  # adversarial problem
            def constraintsTree_1(model, j):
                return sum(model.x[i] * coeff.loc[j, i] for i in F_countefactual_features) + model.w[(outcome, node_ids[j])] <= intercept.iloc[
            ...
            def constraintsTree_1(model, j, s):
                return sum((model.x[i] + S[s][i]) * coeff.loc[j, i] for i in F_countefactual_features) <= intercept.iloc[j] + M * (
...
def constraints_rf(model, outcome, forest_table, F_countefactual_features, rho, unc_type, lb, ub, adversarial_algorithm_bool, S, adv_prob):
    """
    ...
    if not adversarial_algorithm_bool:
        for i, t in enumerate(T):
            ...
            constraints_tree(model, t, tree_table, F_countefactual_features, rho, unc_type, lb=None, ub=None,
                             adversarial_algorithm_bool=False, S=[], adv_prob=False)
...
def constraints_gbm(model, outcome, gbm_table, F_countefactual_features, rho, unc_type, lb, ub, adversarial_algorithm_bool, S, adv_prob):
    """
    ...
    if not adversarial_algorithm_bool:
        for i, t in enumerate(T):
            tree_table = gbm_table_temp.loc[gbm_table_temp['Tree_id'] == t, :].drop(
                ['Tree_id', 'initial_prediction', 'learning_rate'], axis=1, inplace=False)
            constraints_tree(model, t, tree_table, F_countefactual_features, rho, unc_type, lb=None, ub=None,
                             adversarial_algorithm_bool=False, S=[], adv_prob=False)

        def constraint_gbm(model):
            return model.y[outcome] == np.unique(gbm_table_temp['initial_prediction']).item() + np.unique(
                gbm_table_temp['learning_rate']).item() * quicksum(model.y[j] for j in T)
        ...
        if adv_prob:
            for i, t in enumerate(T):
                tree_table = gbm_table_temp.loc[gbm_table_temp['Tree_id'] == t, :].drop(
                    ['Tree_id', 'initial_prediction', 'learning_rate'], axis=1, inplace=False).reset_index(drop=True,
                                                                                                           inplace=False)
                constraints_tree(model, t, tree_table, F_countefactual_features, rho, unc_type, lb=None, ub=None,
                                 adversarial_algorithm_bool=True, S=S, adv_prob=True)
            ...
            def constraint_gbm(model, s):
                return model.y[(outcome, s)] == np.unique(gbm_table_temp['initial_prediction']).item() + np.unique(
                    gbm_table_temp['learning_rate']).item() * quicksum(model.y[(j, s)] for j in T)
...
def constraints_mlp(model, outcome, weights, F_countefactual_features, lb, ub, adv_prob, S, M_l=-1e2, M_u=1e2):
    """
        This function generates the constraints for a neural network.
    """
    if adv_prob:
        nodes_input = range(len(F_countefactual_features))
        v_input = [model.x[i] for i in F_countefactual_features]
        ...
        nodes_input = range(len(F_countefactual_features))
        v_input = [[model.x[i] + S[s][i] for i in F_countefactual_features] for s in range(len(S))]
        max_layer = max(weights['layer'])
        for l in range(max_layer + 1):
            df_layer = weights.query('layer == %d' % l)
            max_nodes = [k for k in df_layer.columns if 'node_' in k]
...
def adv_problem(model, outcome, clf_type, mfile, F_countefactual_features, x_, lb, ub, rho, unc_type):
    """
    ...
    if clf_type == 'cart':
        model = constraints_tree(model, 'prediction', mfile, F_countefactual_features, rho, unc_type, lb, ub, adversarial_algorithm_bool=True,
                                 S=[], adv_prob=True)
    elif clf_type == 'mlp':
        model = constraints_mlp(model, 'prediction', mfile, F_countefactual_features, lb, ub, adv_prob=True, S=[])
    elif clf_type == 'rf':
        model = constraints_rf(model, 'prediction', mfile, F_countefactual_features, rho, unc_type, lb, ub, adversarial_algorithm_bool=True,
                               S=[], adv_prob=True)
    elif clf_type == 'gbm':
        model = constraints_gbm(model, 'prediction', mfile, F_countefactual_features, rho, unc_type, lb, ub, adversarial_algorithm_bool=True,
                                S=[], adv_prob=True)
    if unc_type == 'l2':
        def aux_uncert(model):
            return sum((x_[i].item() - model.x[i]) ** 2 for i in F_countefactual_features) <= rho ** 2
        ...
        model.add_component(outcome + '_aux_unc_1', Constraint(F_countefactual_features, rule=aux_unc_1))

        def aux_unc_2(model, i):
            return model.x[i] >= x_[i].item() - rho

        model.add_component(outcome + 'aux_unc_2', Constraint(F_countefactual_features, rule=aux_unc_2))
    ...
    if (results.solver.status == SolverStatus.ok) and (
        ...
        solution = [value(model.x[i]) for i in F_countefactual_features]
        ...
        for i in range(number_of_solutions):
            ...
            vars_name_x = [opt.get_var_attr(model.x[i], 'VarName') for i in F_countefactual_features]
...
def master_problem(model, u, outcome, clf_type, mfile, F_countefactual_features, rho, unc_type, lb, ub, S, time_limit=1000):
    """
        This function defines and solves the master problem.
    """
    model_temp = add_objective(model, u, 'l1')
    if clf_type == 'cart':
        master_model = constraints_tree(model_temp, outcome, mfile, F_countefactual_features, rho, unc_type, lb, ub,
                                        adversarial_algorithm_bool=True, S=S, adv_prob=False)
    elif clf_type == 'mlp':
        master_model = constraints_mlp(model_temp, outcome, mfile, F_countefactual_features, lb, ub, adv_prob=False, S=S)
    elif clf_type == 'rf':
        master_model = constraints_rf(model_temp, outcome, mfile, F_countefactual_features, rho, unc_type, lb, ub,
                                      adversarial_algorithm_bool=True, S=S, adv_prob=False)
    elif clf_type == 'gbm':
        master_model = constraints_gbm(model_temp, outcome, mfile, F_countefactual_features, rho, unc_type, lb, ub,
                                       adversarial_algorithm_bool=True, S=S, adv_prob=False)
    ...
    opt = SolverFactory('gurobi')
    try:
        opt.solve(master_model, timelimit=time_limit)
    except:
        return None, None
    #     opt.solve(master_model)
    end_time_master = time.time()
    sol_master = pd.DataFrame([[value(master_model.x[i]) for i in F_countefactual_features]], columns=F_countefactual_features)
    print('solution master', [value(master_model.x[i]) for i in F_countefactual_features], 'generated in ',
...
def adversarial_algorithm(model, outcome, clf_type, save_path, u, mfile, F_countefactual_features, rho, unc_type, lb, ub, time_limit):
    """
    ...
    S = [{j: 0 for j in F_countefactual_features}]
    iterations = 0
    # eps = 0.00001
    eps = 0
    ...
    while True:
        ...
        #         except:
        #             break
        master_model, sol_master = master_problem(model.clone(), u, outcome, clf_type, mfile, F_countefactual_features, rho, unc_type, lb,
                                                  ub, S, time_limit=time_limit)
        ...
        _, dist_border = find_maxrad(sol_master, clf_type, save_path, F_countefactual_features, [], [], {}, [], [], [],
                                     pred_, unc_type)
        if dist_border is None:
            print("Stopping because an error occured with the optimizer")
            break
        ...
        adv_model, sol_adv, set_sol_adv, status_adv = adv_problem(model.clone(), outcome, clf_type, mfile, F_countefactual_features,
        ...
        if status_adv == True:
            ...
            for temp_sol_adv in set_sol_adv:
                temp_sol_adv = pd.DataFrame([temp_sol_adv], columns=F_countefactual_features)
                S_df = np.subtract(temp_sol_adv, sol_master)
                if {j: S_df[j].item() for j in F_countefactual_features} not in S:
                    S.append({j: S_df[j].item() for j in F_countefactual_features})
                    break
                else:
                    pass
        elif status_adv is None:
            print('Stopping because ADV problem is infeasible')
            break
        else:
            print('Stopping because solver encountered and error')
            break
        iterations += 1
    ...
    if clf_type == 'cart':
        model = constraints_tree(model_temp, outcome, mfile, F_countefactual_features, rho, unc_type, lb, ub, True, S=S, adv_prob=False)
    elif clf_type == 'rf':
        model = constraints_rf(model_temp, outcome, mfile, F_countefactual_features, rho, unc_type, lb, ub, True, S, False)
    elif clf_type == 'mlp':
        model = constraints_mlp(model_temp, outcome, mfile, F_countefactual_features, lb, ub, adv_prob=False, S=S)
    return model, iterations, comp_time_final, sol_master, solutions_master_dict


def add_objective(model, u, obj_type, maxrad=False):
    F_countefactual_features = list(u.columns)

    if obj_type == 'l2':
        def l2norm(model):
            return sum((u[i].item() - model.x[i]) ** 2 for i in F_countefactual_features)

        model.OBJ = Objective(rule=l2norm, sense=minimize)

    elif obj_type == 'linf':
        model.g = Var(['aux_g'], domain=NonNegativeReals, name=['aux_g_%s' % str(i) for i in F_countefactual_features])

        def obj_aux1(model, i):
            return model.g['aux_g'] >= (u[i].item() - model.x[i])

        model.add_component('obj_aux1', Constraint(F_countefactual_features, rule=obj_aux1))

        def obj_aux2(model, i):
            return model.g['aux_g'] >= -(u[i].item() - model.x[i])

        model.add_component('obj_aux2', Constraint(F_countefactual_features, rule=obj_aux2))
        ...
        model.g = Var(F_countefactual_features, domain=NonNegativeReals, name=['aux_g_%s' % str(i) for i in F_countefactual_features])
        ...
        def obj_aux3(model):
            return sum(model.g[i] for i in F_countefactual_features)
...
def add_validity_constraint(model, clf, clf_type, save_path, u, F_countefactual_features, rho, unc_type, adversarial,
                            time_limit):
    """
    ...
    clf_path = save_path + '/' + clf_type + f'/model_{clf_type}_{unc_type}.csv'
    mfile = pd.read_csv(clf_path)
    ...
    if clf_type in ['linear', 'svm']:
        ...
        model = constraints_linear(model, 'prediction', clf.predict(u), mfile, F_countefactual_features, rho, unc_type,
                                   lb, ub)
        ...
        if not adversarial:
            ...
            model = constraints_tree(model, 'prediction', mfile, F_countefactual_features, rho, unc_type, lb, ub,
                                     adversarial_algorithm_bool=False, S=[], adv_prob=False)
            model = add_objective(model, u, unc_type)
            print('validity constraint embedded.')
            opt = SolverFactory('gurobi')
            ...
            model, num_iterations, comp_time, solution_, solutions_master_dict = adversarial_algorithm(model,
                                                                                                       'prediction',
                                                                                                       clf_type,
                                                                                                       save_path, u,
                                                                                                       mfile,
                                                                                                       F_countefactual_features,
                                                                                                       rho,
                                                                                                       unc_type, lb, ub,
                                                                                                       time_limit)
    elif clf_type == 'rf':
        if not adversarial:
            print('embedding validity constraints...')
            model = constraints_rf(model, 'prediction', mfile, F_countefactual_features, rho, unc_type, lb, ub,
                                   adversarial_algorithm_bool=False, S=[], adv_prob=False)
            model = add_objective(model, u, unc_type)
            opt = SolverFactory('gurobi')
            start_opt = time.time()
        ...
        if not adversarial:
            print('embedding validity constraints...')
            model = constraints_gbm(model, 'prediction', mfile, F_countefactual_features, rho, unc_type, lb, ub,
                                    adversarial_algorithm_bool=False, S=[], adv_prob=False)
            model = add_objective(model, u, unc_type)
            opt = SolverFactory('gurobi')
            start_opt = time.time()
        ...
        model, num_iterations, comp_time, solution_, solutions_master_dict = adversarial_algorithm(model, 'prediction',
                                                                                                   clf_type, save_path,
                                                                                                   u, mfile,
                                                                                                   F_countefactual_features,
                                                                                                   rho,
...
def generate(clf, X_train, y_train, save_path, clf_type, task_type, u, F_countefactual_features, F_binary, F_integers,
             F_coherence, F_immutability, F_larger_than, F_positivity, rho,
             unc_type='ball', iterative=False, time_limit=100):
    """
        This function generate the robust counterfactual explanation
    """
    write_tables(save_path, X_train, y_train, clf, clf_type, task_type, unc_type)
    model = base_model(u, F_countefactual_features, F_binary, F_integers, F_coherence, F_immutability, F_larger_than, F_positivity)
    model, num_iterations, comp_time, solution_, solutions_master_dict = add_validity_constraint(model, clf, clf_type,
                                                                                                 save_path, u,
                                                                                                 F_countefactual_features,
                                                                                                 rho,
                                                                                                 unc_type, iterative,
